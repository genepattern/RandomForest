...
| class_weight | Optional string for class weight specification of either of: {"balanced," "balanced_subsample"}, also takes None ("None" in CLI); "balanced" or "balanced_subsample" are not recommended for warm_start if the fitted data differs from the full dataset; (**future implementation:** handle input of dictionary/list of) | None |
